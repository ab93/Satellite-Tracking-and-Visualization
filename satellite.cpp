...
void getSatelliteList()    //Function to get all the satellite list names
{
...
void getTLE(char* satname)   //Function to get the TLE from the satellite names
{
    int i,j;
...
void SetCountSpeed(int value)
{
    speedFactor = value/10.0;   //should be "/10.0"
    //isChanged = true;
}

void CalculateSatelliteParam(SatelliteParam *sat_par)
{
    vector_t zero_vector={0,0,0,0};
    vector_t vel=zero_vector;
    vector_t pos=zero_vector;
...
//    }
    ...
    jul_utc=daynum+2444238.5;
    //jul_utc=sat_daynum+2444238.5;

    jul_epoch=Julian_Date_of_Epoch(tle.epoch);  /* Convert satellite's epoch time to Julian  */
    tsince=(jul_utc-jul_epoch)*xmnpda;          /* and calculate time since epoch in minutes */

//    if(isChanged == true)
//    {
//        tsince = tsince_temp;
//        sat_daynum = sat_daynum + delta;
//        isChanged = false;
//    }

    delta = tsince;

    //tsince = tsince + (COUNT * 0.01);  //****** For incrementing speed ******* //
    tsince = tsince + (COUNT * speedFactor);
    COUNT++;
    printf("\nCOUNT: %d",COUNT);

    tsince_temp = tsince;

    delta = -1 * (delta - tsince);

    age=jul_utc-jul_epoch;

    printf("\ntsince:%f",tsince);
    printf("\nage: %f",tsince/xmnpda);
    printf("\n%s",Daynum2String(daynum + delta));
    //printf("\njdoy:%f\n",Julian_Date_of_Year(2015));
    //exit(0);

    if (isFlagSet(DEEP_SPACE_EPHEM_FLAG))    /* Copy the ephemeris type in use to ephem string. */
        strcpy(ephem,"SDP4");
    else
        strcpy(ephem,"SGP4");

    if (isFlagSet(DEEP_SPACE_EPHEM_FLAG))    /* Call NORAD routines according to deep-space flag. */
            SDP4(tsince, &tle, &pos, &vel);
    else
            SGP4(tsince, &tle, &pos, &vel);


    Convert_Sat_State(&pos, &vel);    /* Scale position and velocity vectors to km and km/sec */



    Magnitude(&vel);    /* Calculate velocity of satellite */
    sat_vel=vel.w;
    ...
    sat_par->coord = pos;
    sat_par->count = COUNT;
    sat_par->velocity = 3600.0 * sat_vel;
    sat_par->date = Daynum2String(daynum+delta);



    printf("\nx,y,z: %f %f %f\n",sat_par->coord.x,sat_par->coord.y,sat_par->coord.z);
    printf("Velocity: %f\n",sat_par->velocity);
    printf("Distance: %f\n",sqrt(pos.x*pos.x + pos.y*pos.y + pos.z*pos.z));

}

SatelliteParam GetSatelliteParam(int x)
{
    int	xponder=0, polarity=0, tshift, bshift;
    char comsat,geostationary=0, aoshappens=0, decayed=0;
    double downlink=0.0, uplink=0.0, downlink_start=0.0,downlink_end=0.0, uplink_start=0.0, uplink_end=0.0;
    ...
    sat_par.name = sat[x].name;

    //strcpy(sat_par.name,(char*)sat[x].name);
    ...
    do
    {
            ...
            CalculateSatelliteParam(&sat_par);

            fk=12756.33*acos(xkmper/(xkmper+sat_alt));
            fm=fk*km2mi;

            sat_par.footprint = fm;
            sat_par.altitude = sat_alt;

            if(io_lat == 'N')
                sat_par.latitude = 1 * sat_lat;
            else
                sat_par.latitude = -1 * sat_lat;

            if(io_lon == 'W')
                sat_par.longitude = 360.0 - sat_lon;
            else
                sat_par.longitude = sat_lon;
...
void startProcess()    // Function to read TLE files and do other initializations
{
    int x, y, z, key=0;
